...
To normalise a selector, `@extend` the respective placeholder listed in the [Rules](#rules) section. Example :
...
Customise the default value before importing `normalizer.scss` :

	$normalizer-attribute-hidden : true;
	
	@import "normalizer";

or ( permanently ) change it in your working copy here — see the [hidden](#hidden) styling section :
...
To handle the <span id="hidden"> `[hidden]` attribute selector </span> see the [Preferences](#preferences) section.
...
For easy telling apart of the normalising rules and the project specific rules, the `End of normalizer.scss` tag is added.
